...
b0 = 0.02;    %parameter0 for cots of investing
...
phi2 = @(a,k,kprime) 0*b0 * k'*ones(1,size(a,1)) +...
     0*10*(( investment(a,k,kprime)./ ( k'*ones(1,size(a,1)) )-delta).^2).*k'*ones(1,size(a,1));
%for d) (4)
...
xlabel('k')
title('d)04) Optimal Invesment for the mean shock only')
